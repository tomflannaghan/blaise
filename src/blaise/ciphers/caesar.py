...
def caesar_crack(
    ...
    """
    results = [(shift, caesar_decrypt(ciphertext, key=shift, is_norm=is_norm)) 
               for shift in range(26)]
    return ngram_top_n(results, top_n=top_n, expected=ngram_dist, n=ngram_n, key=lambda x: x[1])
